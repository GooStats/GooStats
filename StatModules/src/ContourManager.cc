...
void ContourManager::plot_profiles() {
  for (auto var : profiles_vars) {
    ...
    min = 1e10;
    for (double xx = get_var(var)->value; xx < gr->GetX()[gr->GetN() - 1];
         xx += (gr->GetX()[1] - gr->GetX()[0]) / 100) {
      if (fabs(gr->Eval(xx) - 1) < min) {
        min = fabs(gr->Eval(xx) - 1);
        R = xx;
      }
    ...
    std::string legend(label(var) + " " + TextOutputManager::show_numbers(get_var(var)->value, ef) + " [" +
                       TextOutputManager::show_numbers(L - get_var(var)->value, ef) + ", +" +
                       TextOutputManager::show_numbers(R - get_var(var)->value, ef) + "]");
...
TGraph *ContourManager::LLprofile(const std::string &parName) {
  ...
  int Npoint = 40;
  if (GlobalOption()->has("profile_N"))
    Npoint = GlobalOption()->get<double>("profile_N");
...
TObject *ContourManager::LLcontour(const std::string &par1, const std::string &par2, const std::vector<double> &) {
  int Npoint = 16;
  if (GlobalOption()->has("contour_N"))
    Npoint = GlobalOption()->get<double>("contour_N");
...
int ContourManager::get_id(const std::string &parName) const { return getGSFitManager()->get_id(parName); }

std::string ContourManager::label(const std::string &parName) {
  if (GlobalOption()->has("label_" + parName))
    return GlobalOption()->get("label_" + parName);
  return parName;
}
...
void ContourManager::get_par_range(const std::string &parName, double &left, double &right) {
  ...
  if (left < var->lowerlimit) {
    left = var->lowerlimit;
    right += min(var->upperlimit - right, 0.5 * sigma);
  }
  if (right > var->upperlimit) {
    right = var->upperlimit;
    left -= min(left - var->lowerlimit, 0.5 * sigma);
  }
  if (GlobalOption()->has("profile_" + parName + "_min"))
    left = GlobalOption()->get<double>("profile_" + parName + "_min");
  if (GlobalOption()->has("profile_" + parName + "_max"))
    right = GlobalOption()->get<double>("profile_" + parName + "_max");
}
