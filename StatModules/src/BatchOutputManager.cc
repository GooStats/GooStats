...
bool BatchOutputManager::init() {
  ...
  if (gOp->has("unit"))
    TextOutputManager::set_unit(gOp->get("unit"));
  cd();
...
void BatchOutputManager::cd() { static_cast<OutputManager *>(find("OutputManager"))->getOutputFile()->cd(); }
...
#define OPRINTF(...)                        \
  ...
  for (auto var : vars) {
    ...
    if (var->numbins < 0)  // dirty hack
      OPRINTF(" %s\n",
              TextOutputManager::rate(var->name,
                                      var->value,
                                      var->error,
                                      var->upperlimit,
                                      var->lowerlimit,
      ...
      OPRINTF(" %s\n",
              TextOutputManager::qch(var->name,
                                     var->value,
                                     var->error,
                                     var->upperlimit,
                                     var->lowerlimit,
