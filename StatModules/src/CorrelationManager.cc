...
TString CorrelationManager::label(TString parName) {
  if (GlobalOption()->has(("label_" + parName).Data()))
    return GlobalOption()->get(("label_" + parName).Data());
  return parName;
}
void CorrelationManager::print() {
  ...
  for (int i = 1; i <= fNu; ++i) {
    ...
    if (std::find(corr_vars.begin(), corr_vars.end(), fCpnam[i - 1]) == corr_vars.end())
      continue;
  ...
  for (int ii = 1; ii <= fNu; ++ii) {
    ...
    if (std::find(interested_vars.begin(), interested_vars.end(), ii) == interested_vars.end())
      continue;
  ...
  for (int ii = 1; ii <= fNu; ++ii) {
    ...
    for (int jj = 1; jj <= fNu; ++jj) {
      ...
      fMATUvline[j - 1] =
          fVhmat[ndex - 1] / TMath::Sqrt(TMath::Abs(fVhmat[ndi - 1] * fVhmat[ndj - 1])) * (fWerr[i - 1] * fWerr[j - 1]);
...
void CorrelationManager::analyze() {
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    for (size_t j = 0; j < n; ++j) {
      printf(
          "%5.2lf*%s%s", Binv(i, j), label(interested_names[j]).Data(), j == n - 1 || Binv(i, j + 1) < 0 ? " " : "+");
