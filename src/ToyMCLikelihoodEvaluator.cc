...
void ToyMCLikelihoodEvaluator::get_p_value(GSFitManager *gsFitManager,InputManager *manager,double LL,double &p,double &perr) {
  ...
  totalPdf->setFitControl(new BinnedNllFit);
  totalPdf->copyParams();
  totalPdf->cache();
  LLs.clear();
  int n = 0;
