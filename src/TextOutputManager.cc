...
const std::string TextOutputManager::rate(std::string name_,double v,double e,double max,double min,std::string unit,bool penalty ,double penalty_mean ,double penalty_sigma ) {
  ...
  if(e>0) {
    if(((max-v)/(max-min)>0.01)&&(v-min)/(max-min)>0.01) {
      ...
      output<<name<<" = "<<show_numbers(v,effective_digits)<<" #pm "<<
        show_numbers(e,effective_digits)<<" "<<unit;
    } else
...
const std::string TextOutputManager::qch(std::string name_,double v,double e,double max,double min,bool penalty ,double penalty_mean ,double penalty_sigma ) {
...
std::string TextOutputManager::show_numbers(double x,unsigned int d) {
  return Form(Form("%%.%dlf",d),x);
}
std::string TextOutputManager::prettify_speciesName(std::string orig) {
  if(orig.find(".")!=std::string::npos) orig = orig.substr(orig.find_last_of(".")+1);
  ...
  do
  {
    if (isalpha(orig[start]))
    {
      ...
      if (start < orig.size())
      {
        ...
        else if (orig.size() > start + 1 &&
            orig[start] == '-' && isdigit(orig[start + 1]))
        {
          ++start;
          end = orig.find_first_not_of(digits, start);
          massno = orig.substr(start, end - start);
