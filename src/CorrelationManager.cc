...
const OptionManager *CorrelationManager::getGlobalOption() const {
  return static_cast<const InputManager*>(find("InputManager"))->GlobalOption();
}
FitManager *CorrelationManager::getFitManager() {
  return static_cast<GSFitManager*>(find("GSFitManager"))->getFitManager();
}
OutputManager *CorrelationManager::getOutputManager() {
  return static_cast<OutputManager*>(find("OutputManager"));
}

void CorrelationManager::register_vars() {
  if(getGlobalOption()->has("corr_variables")) {
    corr_vars = GooStats::Utility::splitter(getGlobalOption()->query("corr_variables"),":");
    for(auto var : corr_vars)
      std::cout<<"analyzing corr. for ["<<var<<"]"<<std::endl;
  }
}
TString CorrelationManager::label(TString parName) {
  if(getGlobalOption()->has(("label_"+parName).Data())) return getGlobalOption()->query(("label_"+parName).Data());
  return parName;
}
...
void CorrelationManager::analyze() {
  ...
  for(size_t i = 0;i<n;++i) {
    ...
    for(size_t j = 0;j<n;++j) {
      printf("%5.2lf*%s%s",Binv(i,j),label(corr_vars[j]).Data(),j==n-1||Binv(i,j+1)<0?" ":"+");
