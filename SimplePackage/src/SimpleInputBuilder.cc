...
std::string SimpleInputBuilder::loadOutputFileName(int argc,
                                                   const char **argv,
...
std::pair<ConfigsetManager *, std::vector<ConfigsetManager *>> SimpleInputBuilder::buildConfigsetManagers(
    ParSyncManager *parManager, int argc, const char **argv) {
  if (argc < 2) {
    usage(argv);
    throw GooStatsException("cmd argument format not understandable");
  }
...
void SimpleInputBuilder::fillRawSpectrumProvider(RawSpectrumProvider *provider, ConfigsetManager *configset) {
  ...
  for (const auto &txtPair : componentsTxt) {
    ...
    f >> n >> e0 >> de;
    if (!f.good())
      throw GooStatsException("Cannot read n/e0/de from <" + txtPair.txt + ">");
...
void SimpleInputBuilder::createVariables(ConfigsetManager *configset) {
  if (configset->has("components")) {
    ...
    for (const auto &par : pars) {
      ...
      auto var = configset->createVar(par,
                                      configset->getOrConvert(par + "_init"),
                                      configset->getOrConvert(par + "_err"),
                                      configset->getOrConvert(par + "_min"),
                                      configset->getOrConvert(par + "_max"));
...
#include "goofit/Variable.h"
PdfBase *SimpleInputBuilder::recursiveBuild(const std::string &name,
  ...
  if (!pdf)
    throw GooStatsException("Empty pdf");
  dataset->set(name, static_cast<PdfBase *>(pdf));
  return dataset->get<PdfBase *>(name);
}
bool SimpleInputBuilder::buildComponenets(DatasetManager *dataset,
                                          RawSpectrumProvider *provider,
                                          ISpectrumBuilder *spcBuilder) {
...
std::vector<std::shared_ptr<DatasetController>> SimpleInputBuilder::buildDatasetsControllers(
...
bool SimpleInputBuilder::fillDataSpectra(DatasetManager *dataset, RawSpectrumProvider *provider) {
  ...
  for (int i = 0; i < Evis->numbins; ++i)
    binned_data->setBinContent(i, provider->pdf(dataset->fullName())[i]);
