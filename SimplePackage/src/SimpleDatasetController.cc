...
void recursiveSet(const std::string comp, ConfigsetManager *configset, DatasetManager *dataset, bool &useAna,
  ...
  if (!configset->hasVar(comp + "_E")) {
    if(configset->has(comp+"_E_nbins")) {
      auto inner_E = configset->createVar(comp + "_E", 0, 0, configset->get<double>(comp + "_E_min"),
                                          configset->get<double>(comp + "_E_max"));
      inner_E->numbins = configset->get<double>(comp + "_E_nbins");
    } else {
  ...
  if (type == "MC") {
    ...
    if (type == "AnaPeak") {
      dataset->set(comp + "_Epeak", configset->createVar(comp + "_Epeak", configset->get<double>(comp + "_Evis_init"),
                                                         configset->get<double>(comp + "_Evis_err"),
                                                         configset->get<double>(comp + "_Evis_min"),
                                                         configset->get<double>(comp + "_Evis_max")));
    } else {
      useNL = true;
      if (type == "AnaShifted") {
        dataset->set(comp + "_dEvis",
                     configset->createVar(comp + "_dEvis", configset->get<double>(comp + "_dEvis_init"),
                                          configset->get<double>(comp + "_dEvis_err"),
                                          configset->get<double>(comp + "_dEvis_min"),
                                          configset->get<double>(comp + "_dEvis_max")));
      }
...
bool SimpleDatasetController::collectInputs(DatasetManager *dataset) {
  try {
    ...
    Variable *Evis = configset->createVar(configset->get("EvisVariable"), 0, 0, configset->get<double>("Evis_min"),
    ...
    if (useAna) {
      ...
      if (configset->has("feq")) dataset->set("feq", configset->get<double>("feq"));
      else
        dataset->set("feq", 1.0);
      if (useNL) {
        std::vector<Variable *> NL;
        Variable *LY = configset->createVar("LY", configset->get<double>("LY_init"), configset->get<double>("LY_err"),
                                            configset->get<double>("LY_min"), configset->get<double>("LY_max"));
        NL.push_back(LY);
        if (configset->has("NLtype") && configset->get("NLtype") == "expPar") {
          Variable *NL_b =
                  configset->createVar("NL_b", configset->get<double>("NL_b_init"), configset->get<double>("NL_b_err"),
                                       configset->get<double>("NL_b_min"), configset->get<double>("NL_b_max"));
          NL.push_back(NL_b);
          Variable *NL_c =
                  configset->createVar("NL_c", configset->get<double>("NL_c_init"), configset->get<double>("NL_c_err"),
                                       configset->get<double>("NL_c_min"), configset->get<double>("NL_c_max"));
          NL.push_back(NL_c);
          Variable *NL_e =
                  configset->createVar("NL_e", configset->get<double>("NL_e_init"), configset->get<double>("NL_e_err"),
                                       configset->get<double>("NL_e_min"), configset->get<double>("NL_e_max"));
          NL.push_back(NL_e);
          Variable *NL_f =
                  configset->createVar("NL_f", configset->get<double>("NL_f_init"), configset->get<double>("NL_f_err"),
                                       configset->get<double>("NL_f_min"), configset->get<double>("NL_f_max"));
          NL.push_back(NL_f);
        } else {
          Variable *qc1 =
                  configset->createVar("qc1", configset->get<double>("qc1_init"), configset->get<double>("qc1_err"),
                                       configset->get<double>("qc1_min"), configset->get<double>("qc1_max"));
          NL.push_back(qc1);
          Variable *qc2 =
                  configset->createVar("qc2", configset->get<double>("qc2_init"), configset->get<double>("qc2_err"),
                                       configset->get<double>("qc2_min"), configset->get<double>("qc2_max"));
          NL.push_back(qc2);
        }
        dataset->set("NL", NL);
      }
      std::vector<Variable *> res;
      Variable *sdn = configset->createVar("sdn", configset->get<double>("sdn_init"), configset->get<double>("sdn_err"),
                                           configset->get<double>("sdn_min"), configset->get<double>("sdn_max"));
      res.push_back(sdn);
      ...
      Variable *sigmaT = configset->createVar(
              "sigmaT", configset->get<double>("sigmaT_init"), configset->get<double>("sigmaT_err"),
              configset->get<double>("sigmaT_min"), configset->get<double>("sigmaT_max"));
      res.push_back(sigmaT);
      if (configset->get("RPFtype") == "MG") {
        Variable *g2 = configset->createVar("g2", configset->get<double>("g2_init"), configset->get<double>("g2_err"),
                                            configset->get<double>("g2_min"), configset->get<double>("g2_max"));
        res.push_back(g2);
      }
      dataset->set("res", res);
    }
