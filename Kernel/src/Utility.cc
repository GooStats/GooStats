...
namespace GooStats {
  namespace Utility {
    ...
    std::string strip(const std::string &k) {
      ...
      if (pComment == std::string::npos)
        pComment = pool.find("#");
      if (pComment != std::string::npos)
        pool = pool.substr(0, pComment);
    ...
    void save(RawSpectrumProvider *provider, const std::string &name, TH1 *h) {
      ...
      for (auto i = 1; i <= h->GetNbinsX(); ++i)
        pdf.push_back(h->GetBinContent(i));
