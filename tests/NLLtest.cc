...
TEST(GooFit, NLLTest) {
  ...
  } catch (std::exception &ex) { std::cerr << "Caught [" << ex.what() << "]" << std::endl; }
  ...
  for (size_t i = 0; i < results.size(); ++i) {
    for (auto x: results[i]) { EXPECT_NEAR(x.second.LL, references[i][x.first].LL, x.second.LL * 5e-11); }
  }
...
TEST(GooFit, NLLTestFail) {
  ...
  auto reference_obj = static_cast<GooStatsNLLCheck *>(reference_f->Get("gpu"));
  auto references = (reference_obj->get_results());
  EXPECT_NEAR(results_obj->get_finalLL() * 100 + 100, reference_obj->get_finalLL(),
              reference_obj->get_finalLL() * 5e-11);
}