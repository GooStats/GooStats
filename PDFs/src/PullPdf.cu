...
__host__ PullPdf::PullPdf(std::string n, Variable* var, fptype m, fptype s, fptype mt, bool half_)
    : DataPdf(nullptr, n), index(registerParameter(var)), data(m), sigma(s), masstime(mt), half(half_) {
  assert(var);
}

__host__ double PullPdf::calculateNLL() const {
  double ret = pow((host_params[index] - data) / sigma, 2);
  if (half && host_params[index] < data)
    ret = 0;
  // Stirling's approximation: lgamma(n+1) = 0.5*log(2*pi)+(n+0.5)*log(n)-n+O(1/n);
  if (!IsChisquareFit())
    ret = ret / 2 + 0.5 * log(2 * M_PI_L * sigma * masstime * sigma * masstime);
#ifdef NLL_CHECK
  printf(
      "log(L) %.12le pull d %.12le mu %.12le sigma %.12le MT %.12le\n", ret, data, host_params[index], sigma, masstime);
#endif
  return ret;
}
...
std::unique_ptr<fptype[]> PullPdf::fill_random() {
  ...
  if (half && data < host_params[index])
    data = gRandom->Uniform(0, host_params[index]);
  h_ptr[0] = data;
  return h_ptr;
}
...
void PullPdf::cache() { data_backup = data; }
void PullPdf::restore() { data = data_backup; }
