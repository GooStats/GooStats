...
template <>
EXEC_TARGET fptype GetNL<ResponseFunctionPdf::NL::Mach4>(fptype *evt, fptype *p, unsigned int *indices) {
...
template <>
EXEC_TARGET fptype GetNL<ResponseFunctionPdf::NL::Echidna>(fptype *evt, fptype *p, unsigned int *indices) {
  const fptype Eraw = (evt[RO_CACHE(indices[RO_CACHE(indices[0]) + 3])]);  // in MeV
  /* parameters follows the exact order of "registerParameter" */
  /* Non-linearity parameters */
  const fptype ly = RO_CACHE(p[RO_CACHE(indices[_NL_index + 0])]); /* light yield, npe/MeV */
  ...
  const fptype funCher = ((CherTh == 0) || (Eraw < CherTh))
                             ? 0
                             : ((A0 + A1 * xCher + A2 * xCher * xCher + A4 * xCher * xCher * xCher) * (1 + A3 * Eraw));
  return Eraw * ly * (funQch + fCher * funCher);
}
template <>
EXEC_TARGET fptype GetNL<ResponseFunctionPdf::NL::expPar>(fptype *evt, fptype *p, unsigned int *indices) {
  const fptype Eraw = (evt[RO_CACHE(indices[RO_CACHE(indices[0]) + 3])]);  // in MeV
  /* parameters follows the exact order of "registerParameter" */
  /* Non-linearity parameters */
...
template <>
EXEC_TARGET fptype GetMean<ResponseFunctionPdf::Mean::normal>(fptype mu, fptype *, unsigned int *) {
  return mu;
}
template <>
EXEC_TARGET fptype GetMean<ResponseFunctionPdf::Mean::peak>(fptype, fptype *p, unsigned int *indices) {
...
template <>
EXEC_TARGET fptype GetMean<ResponseFunctionPdf::Mean::shifted>(fptype mu, fptype *p, unsigned int *indices) {
  const int _NL_size = RO_CACHE(indices[1]);                          /* number of non-linearity parameters */
  const int _Res_size = RO_CACHE(indices[_NL_index + _NL_size]);      /* number of resolution parameters */
...
template <>
EXEC_TARGET fptype GetVariance<ResponseFunctionPdf::RES::charge>(fptype mu, fptype *p, unsigned int *indices) {
  const int _NL_size = RO_CACHE(indices[1]);       /* number of non-linearity parameters */
