...
void SumPdf::fill_random() {
  ...
  for(unsigned int i = 0; i < numEntries; ++i) {
    long long new_value = gRandom->Poisson(cached_sumV[i]*host_array[i*dimensions+observables.size()+1]);
  //  std::cout<<"before ["<<host_array[i*dimensions + observables.size()+0]<<"] after ["<<new_value<<"] ("<<cached_sumV[i]<<","<<host_array[i*dimensions+observables.size()+1]<<")"<<std::endl;
